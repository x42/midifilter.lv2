MFD_FILTER(cctonote)
	...
	, TTF_IPORT(0, "channelf", "Filter Channel",  0.0, 16.0,  0.0,
			PORTENUMZ("Any")
			DOC_CHANF)
	, TTF_IPORT(1, "mode", "Operation Mode",  0.0, 3.0,  1.0,
			lv2:portProperty lv2:integer; lv2:portProperty lv2:enumeration;
			lv2:scalePoint [ rdfs:label "Fixed key, velocity = CC-value" ; rdf:value 0.0 ] ;
			lv2:scalePoint [ rdfs:label "key = CC-value, fixed velocity (64)" ; rdf:value 1.0 ] ;
			lv2:scalePoint [ rdfs:label "All keys, key = parameter, velocity = CC-value" ; rdf:value 2.0 ] ;
			rdfs:comment "")
	, TTF_IPORT(2, "param", "CC Parameter to intercept",  0.0, 127.0,  0.0,
			lv2:portProperty lv2:integer;
			rdfs:comment "unused in 'all keys' mode."
			)
	, TTF_IPORT(3, "key", "Key (midi-note) to use with fixed-key mode",  0.0, 127.0,  48.0,
